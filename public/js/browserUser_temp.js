document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderUser(tree, basePath, container) {
  ...
  function load(path, container) {
    ...
    fetch(`/${lang}/bulletin/ls?path=` +
      encodeURIComponent(path))
      .then(r => r.json())
      .then(data => renderUser(data, path, container))
      .catch(err => console.error(err));
  }
  /* INITIAL LOAD */
  const lang =
    document.documentElement.lang || 'en';
  fetch(`/${lang}/bulletin/ls`)
    .then(r => r.json())
    .then(data => renderUser(data, '', browser))
    .catch(err => console.error(err));
