...
router.get('/view', requireLogin, async (req, res) => {
  ...
  try {
    ...
    res.render('pages/user/en/contact_temp', {
      layout: 'layouts/user',
      title: 'Contact',
      lang: 'en',
      page: 'contact',
...
router.get('/edit', requireLogin, async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    return res.redirect('/admin/contact?msg=File+error&status=error');
  }
});
router.post('/save', requireLogin, async (req, res) => {
  const { draftFile, content } = req.body;
  if (!draftFile || content === undefined) {
    return res.redirect('/admin/contact?msg=Invalid+data&status=error');
  }
  try {
    await fs.writeFile(
    ...
    console.error(err);
    return res.redirect('/admin/contact?msg=Save+failed&status=error');
  }
});
router.post('/save-exit', requireLogin, async (req, res) => {
  ...
  if (!draftFile || !originalFile || content === undefined) {
    ...
    const status = 'error';
    return res.redirect(
      `/admin/contact?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`
    );
  }
