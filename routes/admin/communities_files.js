...
const COMMUNITIES_ROOT = path.join(APP_ROOT, 'content/communities_files');
...
router.get('/edit', requireLogin, async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    return res.redirect('/admin/communities?msg=File+error&status=error');
  }
});
router.post('/save', requireLogin, async (req, res) => {
  const { draftFile, content } = req.body;
  if (!draftFile || content === undefined) {
    return res.redirect('/admin/communities?msg=Invalid+data&status=error');
  }
  try {
    await fs.writeFile(
    ...
    console.error(err);
    return res.redirect('/admin/communities?msg=Save+failed&status=error');
  }
});
