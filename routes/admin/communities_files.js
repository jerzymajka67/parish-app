...
const HOMILIES_ROOT = path.join(APP_ROOT, 'content/communities_files');
...
router.get('/', requireLogin, (req, res) => {
  tree = {};
  res.render('pages/admin/communities_files', {
    layout: 'layouts/admin',
    title: 'Communities_files - admin',
    lang: 'en',
    page: 'communities_files',
...
router.post('/create-folder', requireLogin, async (req, res) => {
  try {
    ...
    res.redirect(`/admin/communities_files?path=${encodeURIComponent(currentPath)}`);
  } catch (err) {
    console.error(err);
...
router.post('/create-html', requireLogin, async (req, res) => {
   ...
   let currentPath = req.body.currentPath || '';
    try {
      ...
      const status = 'error';
       return res.redirect(`/admin/communities_files?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`);
    }
...
if (folder) {
  ...
  msg += `Deleted folder: ${folder}\n`;
    ...
    res.redirect(`/admin/communities_files?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(messageType)}`);
  }
  res.redirect(`/admin/communities_files?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(messageType)}`);
});
router.post('/load-file', requireLogin, upload.single('html'), async (req, res) => {
  try {
    ...
    const status = 'success';
    res.redirect(`/admin/communities_files?path=${encodeURIComponent(currentPath)}&msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`);
  } catch (err) {
    const msg = err.message || 'Upload failed';
    const status = 'error';
    res.redirect(`/admin/communities_files?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`);
  }
});
router.get('/edit', requireLogin, async (req, res) => {
  ...
  if (!originalFile) {
    return res.redirect('/admin/communities_files?msg=File+name+required&status=error');
  }
  ...
  try {
    ...
    console.error(err);
    return res.redirect('/admin/communities_files?msg=File+error&status=error');
  }
});
router.post('/save', requireLogin, async (req, res) => {
  const { draftFile, content } = req.body;
  if (!draftFile || content === undefined) {
    return res.redirect('/admin/communities_files?msg=Invalid+data&status=error');
  }
  try {
    await fs.writeFile(
    ...
    console.error(err);
    return res.redirect('/admin/communities_files?msg=Save+failed&status=error');
  }
});
router.post('/save-exit', requireLogin, async (req, res) => {
  ...
  if (!draftFile || !originalFile || content === undefined) {
    ...
    const status = 'error';
    return res.redirect(
      `/admin/communities_files?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`
    );
  }
  const filePath = path.join(HOMILIES_ROOT, originalFile);
  try {
