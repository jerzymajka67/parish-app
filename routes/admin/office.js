...
const OFFICE_ROOT = path.join(APP_ROOT, 'content/office');
...
router.get('/view', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(OFFICE_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'en/temp_temp.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
    res.render('pages/user/en/temp_temp', {
      layout: 'layouts/user',
      title: 'Office',
      lang: 'en',
      page: 'office',
      ...
      const status = 'error';
      return res.redirect(`/admin/office?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`);
  }
});
router.get('/ls', requireLogin, async (req, res) => {
  try {
    const entries = await fs.readdir(OFFICE_ROOT, { withFileTypes: true });
...
router.post('/create-html', requireLogin, async (req, res) => {
   let fileName = req.body.fileName;
    ...
    const filePath = path.join(OFFICE_ROOT,  fileName);
    try {
...
router.post('/publish-en', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(OFFICE_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'en/office.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
...
router.post('/publish-es', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(OFFICE_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'es/oficina.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
...
router.post('/rename', requireLogin, async (req, res) => {
  ...
  try {
    ...
    const oldPath = path.join(OFFICE_ROOT, fileName);
    const newPath = path.join(OFFICE_ROOT, newName);
    try {
...
router.get('/edit', requireLogin, async (req, res) => {
  ...
  const filePath = path.join(OFFICE_ROOT, originalFile);
  const backupPath = path.join(OFFICE_ROOT, draftFile);
  try {
    ...
    console.error(err);
    return res.redirect('/admin/office?msg=File+error&status=error');
  }
});
router.post('/save', requireLogin, async (req, res) => {
  ...
  try {
    await fs.writeFile(
      path.join(OFFICE_ROOT, draftFile),
      content,
      'utf8'
    );
    ...
    console.error(err);
    return res.redirect('/admin/office?msg=Save+failed&status=error');
  }
});
router.post('/save-exit', requireLogin, async (req, res) => {
  ...
  if (!draftFile || !originalFile || content === undefined) {
    ...
    const status = 'error';
    return res.redirect(
      `/admin/office?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`
    );
  }

  const filePath = path.join(OFFICE_ROOT, draftFile);
