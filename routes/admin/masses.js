...
const MASSES_ROOT = path.join(APP_ROOT, 'content/masses');
...
router.get('/view', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(MASSES_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'en/masses_temp.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
    res.render('pages/user/en/masses_temp', {
      layout: 'layouts/user',
      title: 'Masses',
      lang: 'en',
      page: 'masses',
...
router.get('/ls', requireLogin, async (req, res) => {
  try {
    const entries = await fs.readdir(MASSES_ROOT, { withFileTypes: true });
...
router.post('/create-html', requireLogin, async (req, res) => {
   let fileName = req.body.fileName;
    ...
    const filePath = path.join(MASSES_ROOT,  fileName);
    try {
...
router.post('/publish-en', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(MASSES_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'en/masses.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
...
router.post('/publish-es', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(MASSES_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'es/misas.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
...
router.post('/rename', requireLogin, async (req, res) => {
  ...
  try {
    ...
    const oldPath = path.join(MASSES_ROOT, fileName);
    const newPath = path.join(MASSES_ROOT, newName);
    try {
...
router.get('/edit', requireLogin, async (req, res) => {
  ...
  const filePath = path.join(MASSES_ROOT, originalFile);
  const backupPath = path.join(MASSES_ROOT, draftFile);
  try {
    ...
    console.error(err);
    return res.redirect('/admin/masses?msg=File+error&status=error');
  }
});
router.post('/save', requireLogin, async (req, res) => {
  ...
  try {
    await fs.writeFile(
      path.join(MASSES_ROOT, draftFile),
      content,
      'utf8'
    );
    ...
    console.error(err);
    return res.redirect('/admin/masses?msg=Save+failed&status=error');
  }
});
router.post('/save-exit', requireLogin, async (req, res) => {
  ...
  if (!draftFile || !originalFile || content === undefined) {
    ...
    const status = 'error';
    return res.redirect(
      `/admin/masses?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`
    );
  }

  const filePath = path.join(MASSES_ROOT, draftFile);
