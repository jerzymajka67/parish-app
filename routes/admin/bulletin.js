...
const ABOUT_ROOT = path.join(APP_ROOT, 'content/bulletin');
...
router.post('/publish-en', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(ABOUT_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'en/bulletin.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
...
router.post('/publish-es', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(ABOUT_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'es/boletin.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
...
router.get('/edit', requireLogin, async (req, res) => {
  ...
  if (!originalFile) {
    return res.redirect('/admin/bulletin?msg=File+name+required&status=error');
  }
  ...
  try {
    ...
    console.error(err);
    return res.redirect('/admin/bulletin?msg=File+error&status=error');
  }
});
router.post('/save', requireLogin, async (req, res) => {
  const { draftFile, content } = req.body;
  if (!draftFile || content === undefined) {
    return res.redirect('/admin/bulletin?msg=Invalid+data&status=error');
  }
  try {
    await fs.writeFile(
    ...
    console.error(err);
    return res.redirect('/admin/bulletin?msg=Save+failed&status=error');
  }
});
router.post('/save-exit', requireLogin, async (req, res) => {
  ...
  if (!draftFile || !originalFile || content === undefined) {
    ...
    const status = 'error';
    return res.redirect(
      `/admin/bulletin?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`
    );
  }
