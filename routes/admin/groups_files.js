...
router.post('/create-html', requireLogin, async (req, res) => {
   ...
   let currentPath = req.body.currentPath || '';
    try {
      ...
      const status = 'error';
       return res.redirect(`/admin/groups_files?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`);
    }
...
if (folder) {
  ...
  msg += `Deleted folder: ${folder}\n`;
    ...
    res.redirect(`/admin/groups_files?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(messageType)}`);
  }
  res.redirect(`/admin/groups_files?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(messageType)}`);
});
router.post('/load-file', requireLogin, upload.single('html'), async (req, res) => {
  try {
    ...
    const status = 'success';
    res.redirect(`/admin/groups_files?path=${encodeURIComponent(currentPath)}&msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`);
  } catch (err) {
    const msg = err.message || 'Upload failed';
    const status = 'error';
    res.redirect(`/admin/groups_files?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`);
  }
});
router.get('/edit', requireLogin, async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    return res.redirect('/admin/groups_files?msg=File+error&status=error');
  }
});
router.post('/save', requireLogin, async (req, res) => {
  const { draftFile, content } = req.body;
  if (!draftFile || content === undefined) {
    return res.redirect('/admin/groups_files?msg=Invalid+data&status=error');
  }
  try {
    await fs.writeFile(
    ...
    console.error(err);
    return res.redirect('/admin/groups_files?msg=Save+failed&status=error');
  }
});
