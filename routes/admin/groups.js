...
const GROUPS_ROOT = path.join(APP_ROOT, 'content/groups');
...
router.get('/view', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(GROUPS_ROOT, fileName);
...
router.get('/ls', requireLogin, async (req, res) => {
  try {
    const entries = await fs.readdir(GROUPS_ROOT, { withFileTypes: true });
...
router.post('/publish-en', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(GROUPS_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'en/groups.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
...
router.post('/publish-es', requireLogin, async (req, res) => {
  ...
  try {
    const sourcePath = path.join(GROUPS_ROOT, fileName);
    const targetPath = path.join(TargetDir, 'es/grupos.ejs');
    const html = await fs.readFile(sourcePath, 'utf8');
    const bodyContent = extractBody(html);
    await fs.writeFile(targetPath, bodyContent, 'utf8');
...
router.post('/rename', requireLogin, async (req, res) => {
  ...
  try {
    ...
    const oldPath = path.join(GROUPS_ROOT, fileName);
    const newPath = path.join(GROUPS_ROOT, newName);
    try {
...
router.get('/edit', requireLogin, async (req, res) => {
  ...
  if (!originalFile) {
    return res.redirect('/admin/groups?msg=File+name+required&status=error');
  }
  ...
  try {
    ...
    console.error(err);
    return res.redirect('/admin/groups?msg=File+error&status=error');
  }
});
router.post('/save', requireLogin, async (req, res) => {
  const { draftFile, content } = req.body;
  if (!draftFile || content === undefined) {
    return res.redirect('/admin/groups?msg=Invalid+data&status=error');
  }
  try {
    await fs.writeFile(
    ...
    console.error(err);
    return res.redirect('/admin/groups?msg=Save+failed&status=error');
  }
});
router.post('/save-exit', requireLogin, async (req, res) => {
  ...
  if (!draftFile || !originalFile || content === undefined) {
    ...
    const status = 'error';
    return res.redirect(
      `/admin/groups?msg=${encodeURIComponent(msg)}&status=${encodeURIComponent(status)}`
    );
  }

  const filePath = path.join(GROUPS_ROOT, draftFile);
